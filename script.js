        ...
        <article class="post-card" data-id="${p.id}" data-type="${fileType}">
            ...
            <div class="card-footer">
                ...
                <div class="card-actions">
                    <button class="action-btn comments-btn" data-post-id="${p.id}"><i class="fa-regular fa-comment"></i> <span>${p.comment_count || 0}</span></button>
                </div>
            </div>
        </article>
    `;
...
function renderPosts(list) {
    ...
    grid.querySelectorAll('.comments-btn').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            e.stopPropagation();
            const postId = btn.dataset.postId;
            const post = postsCache.find(p => Number(p.id) === Number(postId));
            currentPost = post;
...
async function fetchPosts() {
    const { data, error } = await sb.from('posts').select('*, comment_count:comments(count)').order('created_at', { ascending: false });
    if (error) {
...
try {
    sb.channel('posts-channel')
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'comments' }, payload => {
            // Saat ada komen baru, perbarui seluruh daftar postingan
            fetchPosts();
        })
        .subscribe();
} catch (e) {
